...
class DependencyExtractor:
    ...
    def extract(self) -> dict:
        ...
        # static/ directory lives next to scripts/ at the project root
        static_dir = self.config.config_dir.parent.parent / "static"
        ...
        for pkg in pyodide_packages:
            ...
            if version:
                ...
                if ".dev" in version:
                    # Look for a matching local wheel in static/ to serve via Pyodide
                    local_wheels = sorted(static_dir.glob(f"{base_name.replace('-', '_')}-*.whl"))
                    if not local_wheels:
                        local_wheels = sorted(static_dir.glob(f"{base_name}-*.whl"))
                    if local_wheels:
                        wheel_name = local_wheels[-1].name
                        pkg["pip"] = f"/{wheel_name}"
                        pkg["pre"] = False
                        print(f"    {base_name} {version} is dev — using local wheel: {wheel_name}")
                    else:
...
class TypeScriptGenerator:
    ...
    def write_acausal_blocks(self, blocks: dict, config: dict, import_paths: dict[str, str]) -> None:
        """Write acausal-blocks.ts file."""
        lines = [
        ...
        lines.append("export const acausalBlockConfig: Record<string, string[]> = {")
        for category, block_names in config.items():
            names_str = ", ".join(f'"{name}"' for name in block_names)
            lines.append(f'  "{category}": [{names_str}],')
        lines.append("};")
        ...
        lines.append("export const acausalBlockImportPaths: Record<string, string> = {")
        for block_name, path in sorted(import_paths.items()):
            lines.append(f'  "{block_name}": "{path}",')
        lines.append("};")
        lines.append("")

        output_path = self.output_dir / "nodes" / "generated" / "acausal-blocks.ts"
        output_path.parent.mkdir(parents=True, exist_ok=True)
        output_path.write_text("\n".join(lines), encoding="utf-8")
        print(f"Generated: {output_path}")
...
# Maps domain variable-name sets to domain string identifiers
_DOMAIN_SIGNATURES: list[tuple[frozenset[str], str]] = [
    (frozenset({"V", "I"}),            "electrical"),
    (frozenset({"P", "mdot", "h"}),    "gas_stream"),
    (frozenset({"P", "T", "mdot", "Q"}), "gas"),
    (frozenset({"F", "v"}),            "mechanical"),
    (frozenset({"T", "Q_dot"}),        "thermal"),
    (frozenset({"P", "q"}),            "hydraulic"),
]


def _detect_domain(port_variables: set[str]) -> str:
    """Infer domain string from the set of port variable names."""
    for signature, domain in _DOMAIN_SIGNATURES:
        if signature.issubset(port_variables):
            return domain
    return "unknown"


class AcausalExtractor:
    """Extract acausal component metadata from pathsim.acausal.lib.*"""

    # Components that require specific kwargs to instantiate (no all-default signature)
    _FORCED_KWARGS: dict[str, dict[str, Any]] = {
        "InertialOrificeStream": {"Cd": 0.7, "A": 1e-4, "L_m": 0.1},
        "VoltageSource":         {"func": lambda t: 0.0},
        "CurrentSource":         {"func": lambda t: 0.0},
    }

    def __init__(self, config_dir: Path):
        self.config_dir = config_dir
        self.acausal_config_path = config_dir / "pathsim-acausal" / "blocks.json"

    def _load_config(self) -> dict:
        if not self.acausal_config_path.exists():
            raise FileNotFoundError(f"Acausal config not found: {self.acausal_config_path}")
        with open(self.acausal_config_path, encoding="utf-8") as f:
            return json.load(f)

    def _import_class(self, class_name: str, import_path: str) -> type:
        """Dynamically import a component class from an import path."""
        module = importlib.import_module(import_path)
        return getattr(module, class_name)

    def _get_port_names_and_domain(self, cls: type, class_name: str) -> tuple[list[str], str]:
        """Instantiate the component and read its ports dict."""
        forced = self._FORCED_KWARGS.get(class_name, {})
        try:
            obj = cls(**forced)
        except Exception as e:
            print(f"    Warning: could not instantiate {class_name} for port inspection: {e}")
            return [], "unknown"

        ports = getattr(obj, "ports", {})
        port_names = list(ports.keys())

        # Collect all variable names across all ports to detect domain
        all_vars: set[str] = set()
        for port in ports.values():
            domain_obj = getattr(port, "domain", None)
            if domain_obj is not None:
                for pv in getattr(domain_obj, "variables", []):
                    all_vars.add(getattr(pv, "name", ""))

        domain = _detect_domain(all_vars)
        return port_names, domain

    def _extract_params(self, cls: type) -> dict[str, dict]:
        """Extract constructor params (excluding self and name)."""
        try:
            sig = inspect.signature(cls.__init__)
        ...
        for param_name, param in sig.parameters.items():
            ...
            # 'media' params are complex Python objects — represent as any with no default
            if param_name == "media":
                params[param_name] = {
                    "type": "any",
                    "default": None,
                    "description": extract_param_description(cls.__doc__ or "", param_name)
                            or "gas media properties (e.g. IDEAL_AIR)",
                }
                continue

            # 'spec' params (OrificeSpec objects) — also any
            if param_name == "spec":
                params[param_name] = {
                    "type": "any",
                    "default": None,
            ...
            # Callable defaults (e.g. lambda) — type callable, no default
            ...
            param_type = infer_param_type(default, param_name)
            formatted = format_default(default)
    ...
    def extract_all(self) -> tuple[dict, dict, dict[str, str]]:
        """
        Returns:
            acausal_blocks  – dict of blockName -> block descriptor
            acausal_config  – dict of category -> [blockNames]
            import_paths    – dict of blockName -> Python import path
        """
        config = self._load_config()
        import_path_base = config.get("importPath", "pathsim.acausal.lib")
        categories: dict[str, list[str]] = config.get("categories", {})

        acausal_blocks: dict[str, Any] = {}
        import_paths: dict[str, str] = {}

        for category, block_names in categories.items():
            # Map category to import sub-path
            if category == "Electrical":
                import_path = f"{import_path_base}.electrical"
            elif category in ("Gas", "GasStream"):
                import_path = f"{import_path_base}.gas"
            else:
            ...
            for block_name in block_names:
                ...
                try:
                    cls = self._import_class(block_name, import_path)
                except (ImportError, AttributeError) as e:
                    print(f"    Warning: could not import {block_name} from {import_path}: {e}")
                    continue

                port_names, domain = self._get_port_names_and_domain(cls, block_name)
                params = self._extract_params(cls)

                # Detect variable-port parameter
                variable_ports = None
                if "n_ports" in params:
                    variable_ports = "n_ports"

                docstring = cls.__doc__ or ""
                description = extract_first_line(docstring)
                docstring_html = rst_to_html(docstring)

                acausal_blocks[block_name] = {
                    "blockClass": block_name,
                    "description": description,
                    "docstringHtml": docstring_html,
                    "domain": domain,
                    "importPath": import_path,
                    "ports": [{"name": p, "domain": domain} for p in port_names],
                    "params": params,
                    **({"variablePorts": variable_ports} if variable_ports else {}),
                }
                import_paths[block_name] = import_path

        return acausal_blocks, {cat: names for cat, names in categories.items()}, import_paths
...
"""
    ...
    parser.add_argument("--acausal", action="store_true", help="Extract acausal components only")
    args = parser.parse_args()

    # If no specific flag, extract all
    extract_all = not (args.blocks or args.events or args.simulation or args.deps
                       or args.registry or args.validate or args.acausal)
    ...
    if extract_all or args.acausal:
        print("\nExtracting acausal components...")
        acausal_blocks, acausal_config, acausal_import_paths = AcausalExtractor(config_dir).extract_all()
        generator.write_acausal_blocks(acausal_blocks, acausal_config, acausal_import_paths)
        print(f"  Total: {len(acausal_blocks)} acausal components")
